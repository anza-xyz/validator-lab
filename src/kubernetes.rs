...
impl<'a> Kubernetes<'a> {
    ...
    pub fn create_bootstrap_validator_replica_set(
        ...
        let mut command = vec![
            "/home/solana/k8s-cluster-scripts/bootstrap-validator-startup-script.sh".to_string(),
        ];
    ...
    pub fn create_metrics_secret(&self) -> Result<Secret, Box<dyn Error>> {
        ...
        if let Some(metrics) = &self.metrics {
            ...
            return Err(
                format!("Called create_metrics_secret() but metrics were not provided.").into(),
            );
        }
    ...
    fn set_load_balancer_environment_variables(&self) -> Vec<EnvVar> {
        vec![
            k8s_helpers::create_environment_variable(
                ...
                Some(
                    "bootstrap-and-rpc-lb-service.$(NAMESPACE).svc.cluster.local:8899".to_string(),
                ),
                None,
            ),
            k8s_helpers::create_environment_variable(
                ...
                Some(
                    "bootstrap-and-rpc-lb-service.$(NAMESPACE).svc.cluster.local:8001".to_string(),
                ),
                None,
            ),
            k8s_helpers::create_environment_variable(
                ...
                Some(
                    "bootstrap-and-rpc-lb-service.$(NAMESPACE).svc.cluster.local:9900".to_string(),
                ),
                None,
            ),
        ]
